...
URLProvider.prototype.save = function(params, callback) {
  var newurl = new URL({url: params['url'], created_at: new Date()});
  
  if(params['code']){newurl.code = params['code'];}
  else{
    newurl.code = base36.numtosxg(Math.floor(Math.random()*Math.pow(2,32)));
  }
